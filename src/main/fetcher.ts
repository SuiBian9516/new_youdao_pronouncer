    ...
    let handler = async () =>{
      try{
        if(!Utils.checkFileExistence(path)){
          let response = await axios.post(this.audioHost,new URLSearchParams(this.createParam(request)),{
  ...
  public addImageTask(request:string,path:string){
    let handler = async () =>{
      try{
        ...
        if (axios.isAxiosError(error)) {
          Logger.getInstance().error("Error when fetching image from pixabay: " + error.message, "fetcher");
        }
        Logger.getInstance().error("Unknown error: " + error.message, "fetcher");
      }
    }
