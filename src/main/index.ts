...
ipcMain.handle('project:create', async (_, projectInfo) => {
  try {
    const { name, path: projectPath } = projectInfo;
    const projects = pronouncer.getProjects();
    ...
    try {
      pronouncer.newProject(name, join(projectPath,name));
...
ipcMain.handle('project:generate-video', async (_, projectName, outputPath) => {
  try {
    ...
    const defaultOutputPath = join(app.getPath('videos'), `${projectName}_${new Date().getTime()}.mp4`);
    const finalOutputPath = outputPath || defaultOutputPath;

    Logger.getInstance().info(`Start to generate video: ${projectName}, output path: ${finalOutputPath}`, 'main');
    
    let {appKey,secretKey} = pronouncer.getYoudaoKeys();
    let pixabayKey = pronouncer.getPixabayKey();
    let db = project.getDatabase();
    let fetcher = db.initFetcher(appKey,secretKey,pixabayKey,project.getAudioCachePath(),project.getImageCachePath());
